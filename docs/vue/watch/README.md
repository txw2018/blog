...
```
然后都是调用了`createWatcher(vm, key, handler)`
```flow js
...
```
在`createWatcher`里面做了`handler`是对象或者字符串处理，最终调用`vm.$watch(expOrFn, handler, options)`
...
然后immediate设置为true的时候，在执行$watch的时候就会直接执行`invokeWithErrorHandling(cb, vm, [watcher.value], vm, info)`
